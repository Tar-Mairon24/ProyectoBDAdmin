...
from gui.ui_generos import Ui_Genders

class Generos(QWidget):
    def __init__(self, parent=None):
        super(Generos,self).__init__(parent)
        self.ui = Ui_Genders()
    ...
    def guardar(self):
        genero = self.ui.genero_text.text()
        if genero == '':
            QMessageBox.warning(self, "Error", "Campo vacio")
        elif len(genero) > 45:
            QMessageBox.warning(self, "Error", "Genero tiene muchos caracteres")
        elif genero.isnumeric():
            QMessageBox.warning(self, "Error", "Genero no puede ser un numero")
        elif self.existeGenero(self.conexion, genero):
            QMessageBox.warning(self, "Error", "El género ya existe")
        elif self.ui.id_label.text() == 'Id:':
            self.insertar(self.conexion, genero)
            self.actualizarCampos()
        else:
            self.update(self.conexion, genero, self.ui.id_label.text())
            self.actualizarCampos()
  
    def insertar(self, conexion, genero):
        try:
            conexion.connect()
            sql = f'SELECT id_genero FROM Genero ORDER BY id_genero DESC LIMIT 1'
            result = conexion.select(sql)
            if(len(result) > 0):
                id = int(result[0][0]) + 1
            else:
                id = 1
            sql = f"INSERT INTO Genero VALUES({id}, '{genero}')"
